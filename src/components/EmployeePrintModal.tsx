  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-900/80 backdrop-blur-sm flex justify-center p-4 print:p-0 print:bg-white print:block print:static print:z-auto">
      <div className="max-w-4xl w-full my-6 print:m-0 print:w-full print:max-w-none">
        ...
        <div className="print-document-sheet bg-white text-slate-900 font-sans leading-snug p-6 rounded-2xl shadow-2xl border border-slate-200 print:border-none print:shadow-none print:rounded-none print:p-2 print:m-0 print:text-[11px]">
          {/* Print Header */}
                ...
                <tbody>
                  {employee.salaryHistory.map(sal => (
                    <tr key={sal.id} className="border-b">
                      ...
                      <td className="p-1 font-mono">₱{Number(sal.previousSalary || 0).toLocaleString()}</td>
                      <td className="p-1 font-mono font-bold text-emerald-800">₱{Number(sal.newSalary || 0).toLocaleString()}</td>
